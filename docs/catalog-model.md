...
## Real identities (production)

`examples/org.yaml` is the **local-development placeholder** (it carries
the `guest` fixture the guest provider signs in as). Real operator
identities live in `examples/org.provisioned.yaml`, generated by
`scripts/provision-identities.mjs` from a short list of GitHub logins +
group memberships - see
[identity-and-access.md](./identity-and-access.md)'s "Provisioning users
and admins". `app-config.production.yaml` imports
`examples/org.provisioned.yaml` (not `examples/org.yaml`), so the `guest`
fixture never exists in the production catalog.

...
All three entities are owned by `group:default/guests`, which comes from
the stock `examples/org.yaml` generated by `create-app`. This is a
**clearly marked placeholder** - there is no real GitHub organization or
team available yet. Replace `group:default/guests` with a real
...
catalog locations entirely) registers two additional `type: file`
locations beyond the stock examples:
